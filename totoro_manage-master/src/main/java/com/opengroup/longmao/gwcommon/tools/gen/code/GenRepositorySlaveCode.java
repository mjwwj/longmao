...
/**
 * 自动生成RepositorySlave接口
 * 
 * @author Hermit
 *
 */
public class GenRepositorySlaveCode {

	/*
	 * 构造函数
	 */
	public GenRepositorySlaveCode(String tablename, String describe) {
		// 首字母大写
		String bigTableName = DBUtil.bigTableName;
		// 首字母小写
		String smallTableName = DBUtil.smallTableName;
		
		// 开始生成repositorySlave代码
		String repositorySlaveCode = repositorySlaveCode(bigTableName, describe);
		try {
			File directory = new File("");
			String outputPath = directory.getAbsolutePath() + "/src/main/java/"
					+ Constant.PACKAGE_OUTPATH_REPOSITORY_SLAVE.replace(".", "/") + "/" + bigTableName + "RepositorySlave.java";
			FileWriter fw = new FileWriter(outputPath);
			PrintWriter pw = new PrintWriter(fw);
			pw.println(repositorySlaveCode);
			pw.flush();
			pw.close();
		} catch (IOException e) {
			e.printStackTrace();
		}
		System.out.println("【"+tablename+"】表【RepositorySlave接口】生成完毕！");
	}

	/**
	 * 功能：生成RepositorySlave接口主体代码
	 * @param tablename
	 * @param bigTableName
	 ...
	 * @return
	 */
	private String repositorySlaveCode(String bigTableName,String describe) {
		...
		// 导包
		sb.append("package " + Constant.PACKAGE_OUTPATH_REPOSITORY_SLAVE + ";\r\n");
		sb.append("\r\n");
		...
		// 注释部分
		sb.append("/**\r\n");
		sb.append(" * 【" + describe + "】 RepositorySlave接口\r\n");
		sb.append(" *\r\n");
		...
		// 实体部分
		sb.append("public interface " + bigTableName + "RepositorySlave extends BaseRepository<"+bigTableName+", Long> {\r\n\r\n");
		//生成所有方法
