...
//	var $tvFeedbackFormBtn = $tvFeedbackForm.find(':submit');
	...
	function allGoodsTypeInfo(){//查询所有的类型
		var $this = $(this);
		$this.postBtn(CONFIG.addr.findAllGoodsTypeInfo2, $this.data(), function(data) {
			console.log(data);
			$('.type_id_div').html(S.template(data, $('#type_id_html').html()));
		});
	}
	...
	function goodsDelete(){//删除
		...
		S.confirm('你确定要删除该商品?', function() {
			$this.postBtn(CONFIG.addr.deleteGoodsCategoryInfo, $this.data(), function() {
				S.alert('删除成功!');
				$formBtn.click();
			})
		});
	...
	function goodsHandle(){//修改显示弹框
		...
		$this.postBtn(CONFIG.addr.findGoodsCategoryInfo, $this.data(), function(data) {//获取单个商品
			$('select[name=isEnable]').find('option').each(function() {// 展示回填
				if ($(this).val() == data.isEnable) {
					$(this).attr('selected', 'selected');
				}
			});
			$('.type_id_div').find('option').each(function() {// 展示回填
				if ($(this).val() == data.goodsTypeId) {
					$(this).attr('selected', 'selected');
				}
			});
